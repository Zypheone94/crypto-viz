...
* Le scraper de prix CoinDesk s'exécute toutes les heures (configurable)
...
2. **Scraper de prix CoinDesk** : collecte des données de marché (prix, capitalisation, etc.) depuis la page de prix de CoinDesk
...
└── scrapers/            # Spiders Scrapy
    ...
    └── spiders/         # Scrapers individuels
        ...
        └── coindesk_price_spider.py     # Scraper de données de prix crypto
```

### Flux de données

1. **Collecte** : les flux RSS et la page de prix CoinDesk sont interrogés à intervalles réguliers
...
# Plus besoin de clé API CoinGecko - nous scrapons maintenant CoinDesk
# COINGECKO_API_KEY=votre_cle_ici  # Deprecated
```
...
# COINGECKO_API_KEY=votre_cle_coingecko  # Plus nécessaire - utilise CoinDesk maintenant
...
CRYPTO_FETCH_INTERVAL=3600     # Intervalle scraping prix CoinDesk (par défaut : 1 heure)
...
# Page de prix CoinDesk (plus d'API key nécessaire)
COINDESK_PRICE_URL=https://www.coindesk.com/price
```
...
Le spider CoinDesk collecte des données structurées comme suit :

```json
{
  "id": "coindesk_bitcoin_20230615_1200",
